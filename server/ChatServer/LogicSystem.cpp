...
LogicSystem::LogicSystem() :_b_stop(false), _p_server(nullptr) {
	RegisterCallBacks();
	_worker_thread = std::thread(&LogicSystem::DealMsg, this); // 工作线程处理消息
}
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
    ...
    std::cout << rsp.error() << " uid is not found!!!!" << std::endl;
...
void LogicSystem::AuthFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    if (to_ip_value == self_name) {     //直接通知对方有认证通过消息
        auto session = UserMgr::GetInstance()->GetSession(touid); // 存储Session字典中获取
        if (session) {
            Json::Value notify;
            notify["error"] = ErrorCodes::Success;
            notify["fromuid"] = uid;
            notify["touid"] = touid;
            std::string base_key = USER_BASE_INFO + std::to_string(uid);
            auto uiser_info = std::make_shared<UserInfo>();
...
void LogicSystem::DealChatTextMsg(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
    ...
    auto to_ip_key = USERIPPREFIX + to_str;
    std::string to_ip_value = "";
    bool get_ip = RedisMgr::GetInstance()->Get(to_ip_key, to_ip_value);
    if (!get_ip) {
        return;
    }
    auto& cfg = ConfigMgr::Inst();
...
// 处理消息，一个完整的消息节点(msg_id, _recvnode->_data, _recv_node->_cur_len)
void LogicSystem::DealMsg()
{
    for (;;) {
        std::unique_lock<std::mutex> unique_lk(_mutex);
        while (_msg_que.empty() && !_b_stop) { 		//判断队列为空则用条件变量阻塞等待，并释放锁
