...
{
    boost::uuids::uuid  a_uuid = boost::uuids::random_generator()(); // 唯一标识码
...
void CSession::Send(std::string msg, short msgid) {
	std::lock_guard<std::mutex> lock(_send_lock); // 发送锁
	//std::cout << "[Debug]:CSession::Send " << _send_que.size() << std::endl;
	int send_que_size = _send_que.size();
	if (send_que_size > MAX_SENDQUE) { // 发送队列异常，长度超了
		std::cout << "session: " << _session_id << " send que fulled, size is " << MAX_SENDQUE << endl;
		return;
	}

	_send_que.push(make_shared<SendNode>(msg.c_str(), msg.length(), msgid)); // 加入到发送队列中
	if (send_que_size > 0) { // 加入之前不为空则不异步写
		return;
	}
	auto& msgnode = _send_que.front();
	boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len),
...
void CSession::HandleWrite(const boost::system::error_code& error, std::shared_ptr<CSession> shared_self)
{
	try {
		...
		if (!error) {
			std::lock_guard<std::mutex> lock(_send_lock);
			_send_que.pop(); // 队列头部已写完，出队列  ---> 队列不为空则继续异步写
			if (!_send_que.empty()) { 
				auto& msgnode = _send_que.front();
				boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len),
...
//读取指定字节数 目前长度、总长度 基于 _socket async_read_some 实现
...
void CSession::AsyncReadHead(int total_len) { // head = id + len 4字节
	auto self = shared_from_this();
	asyncReadFull(HEAD_TOTAL_LEN, [this, self](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			if (ec) { 
				std::cout << "handle read head error: " << ec.what() << std::endl;
				Close(); // 关闭连接
				_server->ClearSession(_session_id);
				return;
			}
