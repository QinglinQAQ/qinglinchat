...
#include <memory>    // for std::shared_ptr, std::make_shared, etc.
...
private:
	...
	std::mutex _mutex;
	std::condition_variable _consume; // 条件变量 消费者
	bool _b_stop;
	std::map<short, FunCallBack> _fun_callbacks; // 回调函数表
	std::shared_ptr<CServer> _p_server;
	std::unordered_map<int, std::shared_ptr<UserInfo> > _users; // 内存中存储
};
